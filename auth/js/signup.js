...
document.getElementById("login").addEventListener("submit", async (e) => {
    ...
    const res = await createUserWithEmailAndPassword(auth, email, password).catch((err) => {
        ...
        document.getElementById("loginbutton").innerHTML = `<i class="fa-light fa-door-open"></i> Sign up`;
        return null;
    });
    if (res == null) {return;}
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        showNotification(4, `Logged in! <a href="../profile/index.html"><button class="puffy_button primary"><i class="fa-light fa-user"></i> Profile</button></a>`);
    }
});