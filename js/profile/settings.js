...
onAuthStateChanged(auth, async (user) => {
    ...
    if (localStorage.getItem("settings-allowThemes") != null) {
        document.getElementById("allowThemes").checked = localStorage.getItem("settings-allowThemes") == "true" ? true : false;
    }
    if (localStorage.getItem("settings-allowProfileThemes") != null) {
        document.getElementById("allowProfileThemes").checked = localStorage.getItem("settings-allowProfileThemes") == "true" ? true : false;
    }
    document.getElementById("allowThemes").addEventListener("change", () => {
        const toggled = document.getElementById("allowThemes").checked;
        localStorage.setItem("settings-allowThemes", toggled);
    })
    document.getElementById("allowProfileThemes").addEventListener("change", () => {
        const toggled = document.getElementById("allowProfileThemes").checked;
        localStorage.setItem("settings-allowProfileThemes", toggled);
    })

    if (data.profileBlue != null) {
        document.getElementById("featuredblue").children[0].src = "../asset/char/" + data.profileBlue;
    }
    ...
    if (await hasBluekidPlus()) {
        ...
        document.getElementById("monthlyCoins").innerHTML = Math.round((current - new Date()) / (1000 * 60 * 60 * 24));
        document.getElementById("monthlyCoins").title = current;
    ...
    if (document.getElementById("confirmMember") != null)
    {
        ...
        document.getElementById("confirmMembership").addEventListener("click", async () => {
            ...
            document.getElementById("confirmMembership").innerHTML = `<i class="fa-light fa-spinner fa-spin fa-xl"></i> Checking...`;
    ...
    document.getElementById("saveProfileSettings").addEventListener("click", async () => {
        ...
        document.getElementById("saveProfileSettings").innerHTML = `<i class="fa-light fa-floppy-disk"></i> Save all profile settings`;
    })
    document.getElementById("changename").addEventListener("click", async () => {
        document.getElementById("changename").innerHTML = `<i class="fa-light fa-hourglass"></i> Waiting...`;
        const newname = document.getElementById("newname").value;
        const confirm = document.getElementById("confirmnewname").value;
        if (newname != confirm) {
            document.getElementById("changename").innerHTML = `<i class="fa-light fa-shuffle"></i> Change name`;
    ...
    document.getElementById("sendpasswordreset").addEventListener("click", async () => {
        document.getElementById("sendpasswordreset").innerHTML = `<i class="fa-light fa-hourglass fa-spin"></i> Sending...`;
        await sendPasswordResetEmail(auth, user.email);
        showNotification(4, "Email sent! (Check your spam folder)");
        document.getElementById("sendpasswordreset").innerHTML = `<i class="fa-light fa-envelope"></i> Send password reset email`;
    });
    document.getElementById("changemail").addEventListener("click", async () => {
        ...
        document.getElementById("changemail").innerHTML = `<i class="fa-light fa-hourglass"></i> Waiting...`;
