import { onAuthStateChanged, auth, db, doc, getDoc, signOut, getDocs, deleteDoc, setDoc, updateDoc, collection, deleteObject, storageref, storage, listAll, KIT_COVER_LOCATION, query, limit, where, hasBluekidPlus, insertLoadingScreen, finishLoading, updateLoadingInfo } from "../util/firebase.js";
...
async function loadFavorites(udata) {
    return new Promise(async (res, rej) => {
        ...
        for (let i = 0; i < udata.favoriteKits.length; i++) {
            ...
            left_Data.children[2].innerHTML = `<i id="private_indicator" class="fa-light fa-lock"></i> Unshared`;
            switch (data.visibility) {
                case "private":
                    break;
                case "friends":
                    left_Data.children[2].innerHTML = `<i class="fa-light fa-user-group"></i> Friends only`;
                    break;
                case "public":
                    left_Data.children[2].innerHTML = `<i class="fa-light fa-globe"></i> Public`;
                    break;
                default:
                    break;
            }
            ...
            right.children[2].addEventListener("click", async () => {
                right.children[2].innerHTML = `<i class="fa-light fa-gear fa-spin"></i> Working...`;
            ...
            if (authorres.exists()) {
                ...
                left_Data.children[0].innerHTML += ` <i title="The author field is not vaild. This kit is not able to be uploaded online. (To resolve, create a new kit)" class="fa-light fa-globe fa-xs" style="color: #8f0000;"></i>`;
            }
...
function addContextMenuFunctionality(udata, kitid, kitdata) {
    ...
    if (udata.favoriteKits.includes(kitid)) {
        favorite.innerHTML = `<i class="fa-light fa-star"></i>`;
    } else {
        favorite.innerHTML = `<i class="fa-light fa-star"></i>`;
    }
...
async function addKitElementsBasedOnArray(kits) {
    ...
    for (let i = 0; i < kits.length; i++) {
        if (document.getElementById("publicKitsLoading")) document.getElementById("publicKitsLoading").innerHTML = `<p style="font-size: 1.75rem;"><i class="fa-light fa-gear fa-spin fa-sm"></i> Loading... (${i + 1}/${kits.length})</p>`;
        const kit = kits[i];
        ...
        left.getElementsByClassName("kitcreator")[0].innerHTML = `<i class="fa-light fa-users" aria-hidden="true"></i> Created by <a style="color:black;" href="./user/index.html?id=${kit.ownerUid}">${userdata.data().username}</a>`;
        ...
        if (isFavorite) {
            favBtn.innerHTML = `<i class="fa-light fa-star"></i> Favorited`;
        }
        right.getElementsByClassName("favoritekit")[0].addEventListener("click", async () => {
            ...
            if (isFavorite) {
                ...
                favBtn.innerHTML = `<i class="fa-light fa-star"></i> Favorite`;
...
async function loadPublicKits() {
    ...
    if (document.getElementById("publicKitsLoading")) document.getElementById("publicKitsLoading").innerHTML = `<p style="font-size: 1.75rem;"><i class="fa-light fa-gear fa-spin fa-sm"></i> Preforming query...</p>`;
...
onAuthStateChanged(auth, async (user) => {
    ...
    insertLoadingScreen("content", document.getElementById("normalKits"));
    var uid = user.uid;
    ...
    updateLoadingInfo("content", "Retriving kits");
