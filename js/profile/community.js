import { onAuthStateChanged, auth, db, doc, getDoc, signOut, query, where, getDocs, limit, collection, FirebaseHelper, ONLINE_TEXT, updateDoc, OFFLINE_TEXT, hasBluekidPlus, insertLoadingScreen, updateLoadingInfo, finishLoading } from "../util/firebase.js";
...
async function loadFriends() {
    return new Promise(async (resolve, rej) => {
        ...
        for (let i = 0; i < friends.length; i++) {
            ...
            console.log(friend);
            if (friend.deleted == false && friend.data.profileBlue != null) { clone.children[0].src = "../asset/char/" + friend.data.profileBlue; }
...
onAuthStateChanged(auth, async (user) => {
    ...
    updateLoadingInfo("content", "Loading friend requests (1/3)");
    await loadFriendRequests(userData);
    updateLoadingInfo("content", "Loading send requests (2/3)");
    await loadSentRequests(userData);
    updateLoadingInfo("content", "Loading friends (3/3)");
    await loadFriends();
    ...
    updateLoadingInfo("content", "Wrapping up");
    ...
    if (limitreach == false) {
        document.getElementById("addfriend").addEventListener("click", async () => {
            ...
            if (searchuid == user.uid) {
                ...
                document.getElementById("addfriend").innerHTML = `<i class="fa-light fa-user-group"></i> Add friend`;
